package com.springboot.reactive.assignment1.service;

import com.springboot.reactive.assignment1.entity.*;
import com.springboot.reactive.assignment1.repository.EmpSkillRepository;
import com.springboot.reactive.assignment1.repository.EmployeeRepository;
        ...
        return this.employeeRepository.existsByEmpId(employeeRequest.getEmpId())
                .flatMap(aBoolean -> {
                    if(aBoolean){
                        ...
                        return Mono.zip(this.employeeRepository.save(employee),this.empSkillRepository.save(empSkill))
                                .map(t->new EmployeeResponse(t.getT1().getEmpId(),t.getT1().getEmpName(),
                                        t.getT1().getEmpCity(),t.getT1().getEmpPhone(),t.getT2().getJavaExp(),
